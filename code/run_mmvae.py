...
if (args.fold == '0'): # whole data set
    ...
    mmvae.train(s1_train, s2_train, s1_train, s2_train)
    emb_train = mmvae.predict(s1_train, s2_train)

    if (args.writedir == ''):
        emb_save_dir = 'results/MMVAE_'+format(args.integration)+'_integration/mmvae_LS_'+format(args.ls)+'_DS_'+format(args.ds)+'_'+format(args.distance)+'_beta_'+format(args.beta)
    else:
        emb_save_dir = args.writedir+'/MMVAE_'+format(args.integration)+'_integration/mmvae_LS_'+format(args.ls)+'_DS_'+format(args.ds)+'_'+format(args.distance)+'_beta_'+format(args.beta)
    ...
    mmvae.train(s1_train, s2_train, s1_test, s2_test)
    emb_train = mmvae.predict(s1_train, s2_train)
    emb_test = mmvae.predict(s1_test, s2_test)
